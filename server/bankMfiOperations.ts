...
export async function listBankMfiSnapshot(req: BankRequest): Promise<BankSnapshot> {
  ...
  const resources: Array<[keyof BankSnapshot, string, string, string | undefined]> = [
    ...
    ["paymentInstructions", "bank_payment_instructions", "id,instruction_number,payment_type,channel,source_account_id,destination_account_id,amount,currency,provider,msisdn,provider_reference,status,requested_at,confirmed_at,failure_reason,idempotency_key,data,created_at", "requested_at.desc"],
