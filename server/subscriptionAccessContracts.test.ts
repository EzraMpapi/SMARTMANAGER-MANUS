...
const migration = read("supabase/migrations/20260823_048_subscription_access_snapshot.sql");
...
describe("platform subscription access contracts", () => {
  it("reuses the existing subscription schema and exposes only a least-privilege authenticated RPC", () => {
    ...
    expect(migration).toContain("REVOKE ALL ON FUNCTION public.billing_access_snapshot() FROM PUBLIC, anon");
    expect(migration).toContain("GRANT EXECUTE ON FUNCTION public.billing_access_snapshot() TO authenticated");
  ...
  it("keeps onboarding on the database catalog and company trial RPC", () => {
    expect(dashboard).toContain('fetch("/api/billing/catalog")');
    expect(dashboard).toContain('callWorkspaceRpcWithSessionRefresh("billing_start_trial"');
    expect(dashboard).toContain("from the account menu");
    expect(dashboard).toContain("preferredPlanCode");
  });

  it("fails closed when access is unknown, pending, expired, or not server-allowed", () => {
