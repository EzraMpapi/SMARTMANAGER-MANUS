...
The static workflow review found legacy “local save” wording in several handlers, including lead creation, stock adjustment, invoice settlement, expense recording, leave approval, workflow creation, feedback, support, document, VICOBA, community, healthcare, education, pharmacy, banking, and HR paths. The legacy AI direct-executor branch containing several such writes is guarded by an explicit unconditional block; it cannot execute under the active approval-only AI contract. For active direct writes, the shared mutation bus reconciles each bound table from its last confirmed Supabase result after a rejected request, and the shared toast boundary replaces local-success wording with: “The server did not confirm this change. It was not saved; live data has been restored.” Existing integration coverage asserts this boundary. The remaining duplicated handler strings are maintainability debt, not an active false-persistence claim at the user interface boundary.
