...
| P1 | Property Management browser setup clicked a hidden desktop menu button after remote dashboard changes | The test used `count()` rather than visibility before clicking the responsive menu control | Require the close/open menu control to be visible before clicking; isolated test and full 19-test browser suite now pass | PASSED |
