...
The refreshed catalog contains **1,097 foreign-key constraints**. A corrected coverage query classified each FK—including foreign keys referencing `auth` and other non-public schemas—against valid and ready PostgreSQL indexes on the public child table. The leading-column-prefix test now distinguishes true gaps from existing primary-key and composite-index coverage.

| Classification | FK relationships | Tables affected | Deployment interpretation |
|---|---:|---:|---|
| Truly uncovered | **1,003** | **506** | Candidate for workload review; not automatically approved for creation. |
...
| Primary-key leading-prefix coverage | **2** | **2** | Already covered; do not create another index. |
| Unique leading-prefix coverage | **51** | **51** | Already covered by valid unique/composite indexes; do not create another index. |
...
The earlier **1,008** figure was a previous-snapshot strict-detector result. On the refreshed snapshot, the strict unmatched-leading-prefix set is **1,007**; four of those have non-leading composite coverage, leaving **1,003 truly uncovered** relationships. These counts must not be interpreted as proof that 1,003 indexes should be deployed.

## Safety checks

The current catalog contains 520 public tables, and the direct FK/RLS reconciliation found all 1,097 public-child foreign keys on RLS-enabled child relations. The result was `1,097` RLS-enabled, `0` RLS-disabled, and `0` NULL RLS-state foreign keys. No table, index, constraint, policy, trigger, function, storage bucket, or migration-ledger row was changed.
...
The `profiles.company_id` relationship is a deployment candidate only after a representative authorization/profile query plan is captured and the object is rechecked immediately before any future deployment. The `profiles.id` FK points to `auth.users`; it remains a conservative classification result because the classifier cannot validate the non-public target’s key semantics from the public-only index inventory, so it must not trigger automatic index creation.
