...
The published receipt-refresh repair was then verified directly: `POS-20260816-DBA7` retained its confirmed line item after history refresh and exposed **Process Return**. Its one-unit return returned HTTP 200 and the receipt displayed the completed refund. The test product was subsequently archived rather than deleted, preserving the immutable POS sale, return, payment, stock-movement, and audit evidence. Final bounded database verification found the product in **Archived** status with stock restored to three units, two QA sales, and two preserved QA return-item records.
