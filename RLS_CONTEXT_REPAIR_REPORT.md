...
The `/app` render check advances to the normal sign-in screen without a current client-side error. Production build attempts were externally terminated with exit code 143 while Vite was transforming the oversized dashboard source; this is tracked separately from TypeScript, test, browser, and live database validation.
