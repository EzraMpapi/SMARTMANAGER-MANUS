...
MASTER_BOOK_LOGO_STORAGE = "/manus-storage/smart-manager-logo-20260824_aa2c04d3.png"
MASTER_BOOK_OWNER = MANAGED_BRANDING_ASSET_ROOT / "ezra-mpapi-owner.png"
MASTER_BOOK_OWNER_STORAGE = "/manus-storage/ezra-mpapi-owner-20260824_83575e38.png"
...
ARCH_CHAPTERS = [
    ...
    ("Database Architecture", "The repository uses a source-versioned Supabase migration history. The live audit on 24 August 2026 returned 542 tables: 519 public and 23 auth. All 519 public application tables reported RLS enabled; the 7 non-RLS entries are Supabase-managed auth-schema tables, not unprotected public application tables. Table definitions, indexes, constraints, functions, grants, and policies are migration-owned.", "Repository ina migration history ya Supabase yenye version. Audit ya 24 Agosti 2026 ilirudisha tables 542: 519 public na 23 auth. Tables zote 519 za public application zilionesha RLS; entries 7 zisizo na RLS ni tables za ndani zinazosimamiwa na Supabase Auth, si tables za public zilizo wazi."),
...
def live_snapshot() -> dict[str, Any]:
    ...
    return {
        ...
        "public_rls_disabled": sum(n.startswith("public.") and t.get("rls_enabled") is not True for t, n in zip(tables, names)),
        "auth_rls_disabled": sum(n.startswith("auth.") and t.get("rls_enabled") is not True for t, n in zip(tables, names)),
        "auth_rls_disabled_names": [n for t, n in zip(tables, names) if n.startswith("auth.") and t.get("rls_enabled") is not True],
...
def build_markdown() -> str:
    ...
    for i, (title, en, sw) in enumerate(ARCH_CHAPTERS, 1):
        ...
        if title == "Database Architecture":
            out += [md_table(["Live metric", "Value", "Interpretation"], [["Total tables", str(LIVE["table_count"]), "Read-only schema inventory at 24 August 2026"], ["Public tables", str(LIVE["public_count"]), "Application-facing public schema inventory"], ["Auth tables", str(LIVE["auth_count"]), "Supabase Auth schema inventory"], ["RLS enabled", str(LIVE["rls_enabled"]), "Tables reported with RLS enabled"], ["Public RLS disabled", str(LIVE["public_rls_disabled"]), "All public application tables should remain protected"], ["Auth-schema entries without RLS", str(LIVE["auth_rls_disabled"]), "Supabase-managed internal auth tables; do not blanket-enable" ], ["Migration records", str(LIVE["migration_count"]), "Live migration ledger records returned by connector"]]), ""]
        out += ["---", ""]
    ...
    out += ["# PART X — DATABASE DICTIONARY", "", f"The live read-only Supabase inventory returned {LIVE['table_count']} tables ({LIVE['public_count']} public and {LIVE['auth_count']} auth). All {LIVE['public_count'] - LIVE['public_rls_disabled']} public application tables reported RLS enabled; the {LIVE['auth_rls_disabled']} non-RLS entries are Supabase-managed auth-schema tables. The following dictionary records the public table surface without exposing row contents.", ""]
...
def build_typst() -> None:
    ...
    lines.append(typst_table(["Metric", "Value", "Interpretation", "Evidence"], [["Project version", "1.0.0", "Package version", "package.json"], ["Live tables", str(LIVE["table_count"]), f"{LIVE['public_count']} public; {LIVE['auth_count']} auth", "Read-only Supabase inventory"], ["Public RLS enabled", str(LIVE["public_count"] - LIVE["public_rls_disabled"]), "All public application tables reported protected", "Read-only Supabase inventory"], ["Public RLS disabled", str(LIVE["public_rls_disabled"]), "Should remain zero", "Read-only Supabase inventory"], ["Auth-schema entries without RLS", str(LIVE["auth_rls_disabled"]), "Supabase-managed internal auth tables; do not blanket-enable", "Read-only Supabase inventory"], ["Migrations", str(LIVE["migration_count"]), "Records in live ledger", "Read-only Supabase migration list"], ["Security lints", str(len(LIVE["security_lints"])), "WARN/INFO advisor records", "Supabase security advisor"], ["Performance lints", str(len(LIVE["performance_lints"])), "WARN/INFO advisor records", "Supabase performance advisor"]]))
    ...
    lines.append(typst_escape(f"Repository version: 1.0.0 from package.json. Documentation date: {TODAY}. Live Supabase audit: read-only. Tables: {LIVE['table_count']}; public: {LIVE['public_count']}; auth: {LIVE['auth_count']}; public RLS disabled: {LIVE['public_rls_disabled']}; auth-schema entries without RLS: {LIVE['auth_rls_disabled']}; live migration records: {LIVE['migration_count']}; security advisor lints: {len(LIVE['security_lints'])}; performance advisor lints: {len(LIVE['performance_lints'])}."))
    lines.append('')
