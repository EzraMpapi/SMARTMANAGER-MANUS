# 24. TRA Portal — Module Design Specification

**Source ID:** `tra_portal`  
**Experience family:** Tanzania tax compliance  
**Primary roles:** Finance Manager / Tax officer  
...
Design TRA Portal as a role-aware workspace for tanzania tax compliance. The module should open with a clear operational question, expose the minimum data required for the current role, and make status, permission, freshness, and next action obvious.

## Screen reference

| Screen | Main components | Primary action | Secondary action | Data displayed |
|---|---|---|---|---|
| Overview / command center | KPI cards, trend/chart, alert rail, recent activity, quick actions | Open the highest-priority queue | Change scope, export snapshot | TIN, receipt, VFD/EFD status, VAT period, buyer, tax amount, Z report, export |
| List / queue | Search, filters, saved views, sortable table, pagination, bulk bar | Create or open a record | Bulk update, export, clear filters | TIN, receipt, VFD/EFD status, VAT period, buyer, tax amount, Z report, export; owner; status; updated time |
...
| Create / edit form | Sectioned form, required markers, inline help, validation summary | Save draft or submit | Cancel, duplicate, reset | TIN, receipt, VFD/EFD status, VAT period, buyer, tax amount, Z report, export |
