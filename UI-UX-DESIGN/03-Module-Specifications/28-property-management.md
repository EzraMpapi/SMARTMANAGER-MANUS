# 28. Property Management — Module Design Specification

**Source ID:** `property-management`  
**Experience family:** Property operations  
**Primary roles:** Property Manager / Tenant  
...
Design Property Management as a role-aware workspace for property operations. The module should open with a clear operational question, expose the minimum data required for the current role, and make status, permission, freshness, and next action obvious.

## Screen reference

| Screen | Main components | Primary action | Secondary action | Data displayed |
|---|---|---|---|---|
| Overview / command center | KPI cards, trend/chart, alert rail, recent activity, quick actions | Open the highest-priority queue | Change scope, export snapshot | property, unit, landlord, tenant, lease, rent, maintenance, inspection |
| List / queue | Search, filters, saved views, sortable table, pagination, bulk bar | Create or open a record | Bulk update, export, clear filters | property, unit, landlord, tenant, lease, rent, maintenance, inspection; owner; status; updated time |
...
| Create / edit form | Sectioned form, required markers, inline help, validation summary | Save draft or submit | Cancel, duplicate, reset | property, unit, landlord, tenant, lease, rent, maintenance, inspection |
