# 31. Healthcare / Clinic — Module Design Specification

**Source ID:** `healthcare`  
**Experience family:** Clinical operations  
**Primary roles:** Clinic Administrator / Doctor  
...
Design Healthcare / Clinic as a role-aware workspace for clinical operations. The module should open with a clear operational question, expose the minimum data required for the current role, and make status, permission, freshness, and next action obvious.

## Screen reference

| Screen | Main components | Primary action | Secondary action | Data displayed |
|---|---|---|---|---|
| Overview / command center | KPI cards, trend/chart, alert rail, recent activity, quick actions | Open the highest-priority queue | Change scope, export snapshot | patient, appointment, triage, visit, diagnosis, prescription, lab order, billing |
| List / queue | Search, filters, saved views, sortable table, pagination, bulk bar | Create or open a record | Bulk update, export, clear filters | patient, appointment, triage, visit, diagnosis, prescription, lab order, billing; owner; status; updated time |
...
| Create / edit form | Sectioned form, required markers, inline help, validation summary | Save draft or submit | Cancel, duplicate, reset | patient, appointment, triage, visit, diagnosis, prescription, lab order, billing |
