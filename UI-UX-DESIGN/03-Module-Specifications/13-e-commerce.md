# 13. E-Commerce — Module Design Specification

**Source ID:** `ecommerce`  
**Experience family:** Digital commerce  
**Primary roles:** Sales Manager / Operations lead  
...
Design E-Commerce as a role-aware workspace for digital commerce. The module should open with a clear operational question, expose the minimum data required for the current role, and make status, permission, freshness, and next action obvious.

## Screen reference

| Screen | Main components | Primary action | Secondary action | Data displayed |
|---|---|---|---|---|
| Overview / command center | KPI cards, trend/chart, alert rail, recent activity, quick actions | Open the highest-priority queue | Change scope, export snapshot | product, price, inventory, order, customer, checkout state, fulfillment status |
| List / queue | Search, filters, saved views, sortable table, pagination, bulk bar | Create or open a record | Bulk update, export, clear filters | product, price, inventory, order, customer, checkout state, fulfillment status; owner; status; updated time |
...
| Create / edit form | Sectioned form, required markers, inline help, validation summary | Save draft or submit | Cancel, duplicate, reset | product, price, inventory, order, customer, checkout state, fulfillment status |
