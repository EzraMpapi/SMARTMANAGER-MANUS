# 08. Reports — Module Design Specification

**Source ID:** `reports`  
**Experience family:** Insight and exports  
**Primary roles:** CFO / Auditor  
...
Design Reports as a role-aware workspace for insight and exports. The module should open with a clear operational question, expose the minimum data required for the current role, and make status, permission, freshness, and next action obvious.

## Screen reference

| Screen | Main components | Primary action | Secondary action | Data displayed |
|---|---|---|---|---|
| Overview / command center | KPI cards, trend/chart, alert rail, recent activity, quick actions | Open the highest-priority queue | Change scope, export snapshot | report type, date range, filters, schedule, recipients, output format |
| List / queue | Search, filters, saved views, sortable table, pagination, bulk bar | Create or open a record | Bulk update, export, clear filters | report type, date range, filters, schedule, recipients, output format; owner; status; updated time |
...
| Create / edit form | Sectioned form, required markers, inline help, validation summary | Save draft or submit | Cancel, duplicate, reset | report type, date range, filters, schedule, recipients, output format |
