...
The identity center confirmed that the active session is limited to the tenant owner identity and that workspace switching is not enabled by the current backend contract. Switching to an authorized Platform Administrator therefore requires ending this session and signing in through the verified Platform Administrator account flow; no unverified in-app selector is available.
