-- The source-aware function has a default for p_subscription_source, so the legacy
-- 10-argument overload makes positional calls ambiguous. Keep the legacy behavior
-- under a private compatibility name and leave one public RPC entry point.
alter function public.platform_admin_control_action(text, uuid, text, uuid, text, text, text, numeric, text, integer)
  rename to platform_admin_control_action_legacy;

revoke all on function public.platform_admin_control_action_legacy(text, uuid, text, uuid, text, text, text, numeric, text, integer) from public, anon, authenticated;
...
begin
  ...
  if v_action in ('CHANGE_PLAN', 'CANCEL_SUBSCRIPTION', 'RESUME_SUBSCRIPTION', 'EXTEND_SUBSCRIPTION') and v_source = 'tenant' then
    select ts.company_id, ts.status, coalesce(bp.name, bp.code)
     ...
     where ts.id = p_target_id for update;
    ...
    elsif p_extension_days not in (7, 15, 30, 60, 90) then raise exception 'extension must be 7,15,30,60,90' using errcode = '22023';
