-- sales_subscriptions is the live billing source used by the Global Admin control plane.
-- subscription_events.subscription_id references tenant_subscriptions, so keep that nullable
-- relationship unset and preserve the billing source ID in event details.
create or replace function public.platform_admin_control_action(
  p_action text, p_target_id uuid default null, p_reason text default null,
  p_company_id uuid default null, p_plan text default null, p_name text default null,
  p_cycle text default null, p_amount numeric default null, p_status text default null,
...
begin
  ...
  if v_action='CREATE_SUBSCRIPTION' then
    ...
    v_old_status:=v_sub.status; v_old_plan:=v_sub.plan; v_new_status:=v_old_status; v_new_plan:=v_old_plan;
    if v_action='CANCEL_SUBSCRIPTION' then v_new_status='cancelled'; elsif v_action='RESUME_SUBSCRIPTION' then v_new_status='Active'; elsif v_action='CHANGE_PLAN' then if nullif(btrim(coalesce(p_plan,'')),'') is null then raise exception 'plan is required' using errcode='22023'; end if; v_new_plan=btrim(p_plan); elsif v_action='EXTEND_SUBSCRIPTION' then if p_extension_days not in(7,15,30,60,90) then raise exception 'extension must be 7, 15, 30, 60, or 90 days' using errcode='22023'; end if; end if;
