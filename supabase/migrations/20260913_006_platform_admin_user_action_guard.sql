-- Complete the source-aware control RPC for user management.
-- Platform administrators cannot be deactivated by this control plane.
...
declare v_action text:=upper(btrim(coalesce(p_action,''))); v_source text:=lower(coalesce(p_subscription_source,'sales')); v_old_status text; v_new_status text; v_old_plan text; v_new_plan text; v_plan_id uuid; v_company_id uuid; v_role text; v_audit_id uuid;
begin
 ...
 if v_action in ('ACTIVATE_USER','DEACTIVATE_USER') then
   ...
   insert into public.platform_admin_actions(actor_user_id,actor_role,action,target_type,target_id,reason,confirmation_text,details) values(auth.uid(),'platform administrator',v_action,'profile',p_target_id::text,p_reason,'CONFIRM:'||v_action||':'||p_target_id::text,jsonb_build_object('previousRole',v_role,'isActive',v_action='ACTIVATE_USER')) returning id into v_audit_id;
   return jsonb_build_object('action',v_action,'targetId',p_target_id,'auditId',v_audit_id);
 end if;
 if v_action not in ('CHANGE_PLAN','CANCEL_SUBSCRIPTION','RESUME_SUBSCRIPTION','EXTEND_SUBSCRIPTION') then raise exception 'unsupported source-aware action' using errcode='22023'; end if;
 if v_source='tenant' then
   ...
   if v_action='CHANGE_PLAN' then select id into v_plan_id from public.billing_plans where lower(name)=lower(btrim(p_plan)) or lower(code)=lower(btrim(p_plan)) limit 1; if v_plan_id is null then raise exception 'billing plan not found' using errcode='P0002'; end if; v_new_plan:=btrim(p_plan); elsif v_action='CANCEL_SUBSCRIPTION' then v_new_status:='Cancelled'; elsif v_action='RESUME_SUBSCRIPTION' then v_new_status:='Active'; elsif p_extension_days not in(7,15,30,60,90) then raise exception 'extension must be 7,15,30,60,90' using errcode='22023'; else v_new_status:='Active'; end if;
   ...
   select company_id,status,plan into v_company_id,v_old_status,v_old_plan from public.sales_subscriptions where id=p_target_id for update; if v_company_id is null then raise exception 'sales subscription not found' using errcode='P0002'; end if; v_new_status:=v_old_status; v_new_plan:=v_old_plan;
   if v_action='CHANGE_PLAN' then v_new_plan:=btrim(p_plan); elsif v_action='CANCEL_SUBSCRIPTION' then v_new_status:='cancelled'; elsif v_action='RESUME_SUBSCRIPTION' then v_new_status:='Active'; elsif p_extension_days not in(7,15,30,60,90) then raise exception 'extension must be 7,15,30,60,90' using errcode='22023'; else v_new_status:='Active'; end if;
   update public.sales_subscriptions set plan=v_new_plan,status=v_new_status,next_billing_date=case when v_action='EXTEND_SUBSCRIPTION' then greatest(coalesce(next_billing_date,now()),now())+make_interval(days=>p_extension_days) when v_action='RESUME_SUBSCRIPTION' and (next_billing_date is null or next_billing_date<now()) then now()+interval '30 days' else next_billing_date end,updated_at=now() where id=p_target_id;
   insert into public.subscription_events(company_id,subscription_id,event_type,previous_status,new_status,actor_profile_id,actor_type,details) values(v_company_id,null,v_action,v_old_status,v_new_status,auth.uid(),'platform_admin',jsonb_build_object('reason',p_reason,'salesSubscriptionId',p_target_id,'previousPlan',v_old_plan,'newPlan',v_new_plan,'extensionDays',p_extension_days));
 end if;
