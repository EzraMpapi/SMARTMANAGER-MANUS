...
-- An explicit platform-admin permit is authoritative for an Active/Grace row.
-- This prevents a stale legacy expiry timestamp from re-blocking a subscription
-- that an administrator has explicitly resumed, extended, changed, or approved.
...
begin
  ...
  if found then
    ...
    -- The explicit admin permit takes precedence over a stale expiry timestamp,
    -- but only while the subscription remains Active or Grace. Cancelled,
    -- Pending, and other non-active states still fail closed.
    if v_unlimited_access and v_subscription.status in ('Active', 'Grace') then
      v_state := 'Active';
      v_allowed := true;
      v_access_until := coalesce(v_subscription.expires_at, v_subscription.grace_expires_at);
      v_reason := 'Access is active under an explicit platform administrator permit.';
      ...
      select lower(trim(module_key)) as module_key, bool_or(enabled) as enabled
        from (
          ...
          select coalesce(cm.data->>'module_key', cm.name) as module_key,
            ...
            from public.company_modules cm where cm.company_id = v_company_id
        ) sources
  ...
  return jsonb_build_object(
    ...
    'unlimitedAccess', v_unlimited_access,
    'subscription', case when v_subscription.id is null then null else jsonb_build_object('id', v_subscription.id, 'plan_id', v_subscription.plan_id, 'status', v_subscription.status, 'billing_cycle', v_subscription.billing_cycle, 'amount', v_subscription.amount, 'currency', v_subscription.currency, 'started_at', v_subscription.started_at, 'expires_at', v_subscription.expires_at, 'grace_expires_at', v_subscription.grace_expires_at, 'trial_started_at', v_subscription.trial_started_at, 'trial_ends_at', v_subscription.trial_ends_at, 'source_payment_id', v_subscription.source_payment_id, 'metadata', v_subscription.metadata) end,
    'plan', case when v_plan.id is null then null else jsonb_build_object('id', v_plan.id, 'code', v_plan.code, 'name', v_plan.name, 'features', v_plan.features, 'moduleEntitlements', v_effective_modules) end,
