...
BEGIN
  ...
  IF v_transaction_id IS NOT NULL THEN
    RETURN jsonb_build_object('transaction_id', v_transaction_id, 'doc_number', coalesce(v_existing_doc, p_doc_number), 'idempotent_replay', true);
  END IF;
  ...
  VALUES (v_company_id, p_doc_number, 'Completed', p_total, NULL, jsonb_build_object('doc_number', p_doc_number, 'idempotency_key', trim(p_idempotency_key), 'cashier_id', v_user_id, 'cashier', v_cashier, 'subtotal', p_subtotal, 'tax', p_tax, 'total', p_total, 'payment_status', 'Paid'))
  ...
  FOR v_item IN SELECT value FROM jsonb_array_elements(p_items) LOOP
    ...
    UPDATE public.inventory_items
       SET data = jsonb_set(data, '{qty_on_hand}', to_jsonb(v_stock - v_qty), true), updated_at = now()
     WHERE id = v_inventory_id;
    INSERT INTO public.pos_transaction_items (company_id, name, status, amount, notes, data)
    VALUES (v_company_id, v_item_name, 'Completed', v_qty * v_price, NULL, jsonb_build_object('transaction_id', v_transaction_id, 'item_name', v_item_name, 'item_sku', v_sku, 'qty', v_qty, 'price', v_price));
    INSERT INTO public.inventory_stock_movements (company_id, name, status, amount, notes, data)
    VALUES (v_company_id, v_item_name, 'Out', v_qty, NULL, jsonb_build_object('item_id', v_inventory_id, 'item_sku', v_sku, 'movement', 'Out', 'qty', v_qty, 'reference', p_doc_number, 'transaction_id', v_transaction_id));
  END LOOP;
  ...
  FOR v_payment IN SELECT value FROM jsonb_array_elements(p_payments) LOOP
    ...
    VALUES (v_company_id, 'POS payment · ' || p_doc_number, 'Completed', v_applied_amount, NULL, jsonb_build_object('pos_transaction_id', v_transaction_id, 'transaction_ref', p_doc_number, 'method', v_payment_method, 'tendered_amount', v_payment_amount, 'applied_amount', v_applied_amount, 'change_amount', CASE WHEN v_payment_method = 'Cash' THEN v_change ELSE 0 END));
  END LOOP;

  INSERT INTO public.audit_log (company_id, action, module, actor, details, subject, detail)
  VALUES (v_company_id, 'POS sale completed', 'Point of Sale', v_cashier, 'Receipt ' || p_doc_number || ' completed for ' || p_total::text || '.', p_doc_number, jsonb_build_object('transaction_id', v_transaction_id, 'idempotency_key', trim(p_idempotency_key), 'subtotal', p_subtotal, 'tax', p_tax, 'total', p_total));
  ...
  RETURN jsonb_build_object('transaction_id', v_transaction_id, 'doc_number', p_doc_number, 'idempotent_replay', false, 'subtotal', p_subtotal, 'tax', p_tax, 'total', p_total, 'paid', v_computed_paid, 'change', v_change);
END;
$function$;
