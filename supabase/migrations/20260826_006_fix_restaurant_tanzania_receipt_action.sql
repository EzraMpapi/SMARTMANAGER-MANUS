...
DECLARE
  ...
  v_order_id uuid := NULLIF(p_payload->>'sourceId','')::uuid;
  v_profile_id uuid;
  v_receipt_id uuid;
...
BEGIN
  ...
  END IF;
  IF v_source_type <> 'restaurant_order' OR v_order_id IS NULL THEN
    RAISE EXCEPTION 'A native Restaurant order UUID is required for fiscal receipt queueing.' USING ERRCODE='22023';
  END IF;
  IF v_idempotency_key IS NULL THEN
    RAISE EXCEPTION 'An idempotency key is required for fiscal receipt queueing.' USING ERRCODE='22023';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    WHERE o.id=v_order_id AND o.company_id=v_company_id AND o.outlet_id=v_outlet_id
  ) THEN
    RAISE EXCEPTION 'The source Restaurant order does not belong to the verified company outlet.' USING ERRCODE='22023';
  END IF;
  IF v_gross <= 0 OR v_vat < 0 OR v_net < 0 THEN
  ...
  BEGIN
    INSERT INTO public.restaurant_fiscal_receipts(
      company_id,outlet_id,fiscal_profile_id,order_id,internal_reference,status,
      gross_amount,vat_amount,net_amount,currency,idempotency_key,provider_response,
      failure_reason,queued_at
    ) VALUES (
      v_company_id,v_outlet_id,v_profile_id,v_order_id,
      left('restaurant_order:'||v_order_id::text,120),
      'Queued',v_gross,v_vat,v_net,v_currency,v_idempotency_key,
      jsonb_build_object('sourceType',v_source_type,'sourceId',v_order_id,'items',COALESCE(p_payload->'items','[]'::jsonb)),
      NULL,now()
    ) RETURNING id INTO v_receipt_id;
  EXCEPTION WHEN unique_violation THEN
    SELECT * INTO v_existing
      FROM public.restaurant_fiscal_receipts
     WHERE company_id=v_company_id AND (idempotency_key=v_idempotency_key OR order_id=v_order_id)
     LIMIT 1;
    IF FOUND THEN
      RETURN jsonb_build_object('ok',true,'recordId',v_existing.id,'duplicate',true,'status',v_existing.status);
    END IF;
    RAISE;
  END;
  PERFORM public.restaurant_tanzania_audit(
    'RECEIPT_QUEUE','fiscal_receipt',v_receipt_id,
    p_payload || jsonb_build_object('outletId',v_outlet_id,'sourceType',v_source_type,'sourceId',v_order_id)
  );
...
COMMENT ON FUNCTION public.restaurant_tanzania_receipt_action(jsonb) IS 'Queues a tenant-scoped Tanzania fiscal receipt for a native Restaurant order and configured Restaurant fiscal profile; it does not call an external TRA provider.';
