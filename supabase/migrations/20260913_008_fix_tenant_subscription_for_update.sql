-- Fix tenant subscription locking: lock the base row before reading the optional plan. 10-argument control RPC for create/support/invitation/sales actions.
...
begin
 ...
 if v_action in ('CHANGE_PLAN','CANCEL_SUBSCRIPTION','RESUME_SUBSCRIPTION','EXTEND_SUBSCRIPTION') and v_source='tenant' then
   select company_id,status into v_company_id,v_old_status from public.tenant_subscriptions where id=p_target_id for update; select coalesce(bp.name,bp.code) into v_old_plan from public.billing_plans bp join public.tenant_subscriptions ts on ts.plan_id=bp.id where ts.id=p_target_id;
