-- Platform Administrator control plane: bounded, audited, non-destructive operations.
...
as $$
declare v_subscriptions jsonb; v_support jsonb; v_invitations jsonb;
begin
  ...
  select coalesce(jsonb_agg(to_jsonb(s) order by s.updated_at desc), '[]'::jsonb) into v_subscriptions from (select id, company_id, name, plan, cycle, status, amount, start_date, next_billing_date, updated_at from public.sales_subscriptions order by updated_at desc limit 100) s;
  ...
  return jsonb_build_object('subscriptions', v_subscriptions, 'support', v_support, 'invitations', v_invitations, 'generatedAt', now());
end; $$;
...
begin
  ...
  if v_action = 'CREATE_SUBSCRIPTION' then
    ...
    insert into public.sales_subscriptions(company_id, name, plan, cycle, status, amount, start_date, next_billing_date, notes)
    values (p_company_id, coalesce(nullif(btrim(p_name),''), p_plan), btrim(p_plan), coalesce(nullif(btrim(p_cycle),''), 'Monthly'), coalesce(nullif(btrim(p_status),''), 'Active'), p_amount, now(), case when p_extension_days is not null then now() + make_interval(days => p_extension_days) else null end, v_reason)
  ...
  if v_action in ('ACTIVATE_USER','DEACTIVATE_USER') then
    if p_target_id is null then raise exception 'target is required' using errcode = '22023'; end if;
    ...
    if p_target_id is null then raise exception 'target is required' using errcode = '22023'; end if;
    update public.support_tickets set status = case when v_action = 'APPROVE_SUPPORT_TICKET' then 'in_progress' else 'rejected' end, updated_at = now() where id = p_target_id;
    ...
    if p_target_id is null then raise exception 'target is required' using errcode = '22023'; end if;
    update public.team_invitations set status = 'revoked', revoked_at = now(), updated_at = now() where id = p_target_id;
    ...
    select * into v_sub from public.sales_subscriptions where id = p_target_id for update;
