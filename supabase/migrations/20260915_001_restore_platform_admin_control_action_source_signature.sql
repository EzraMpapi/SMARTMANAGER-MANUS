-- Restore the source-aware platform admin RPC signature expected by server/globalAdmin.ts.
-- The existing 10-argument function remains available for legacy actions.
...
begin
  ...
  if v_action in ('CHANGE_PLAN', 'CANCEL_SUBSCRIPTION', 'RESUME_SUBSCRIPTION', 'EXTEND_SUBSCRIPTION')
     and v_source = 'tenant' then
    select ts.company_id, ts.status, coalesce(bp.name, bp.code)
      into v_company_id, v_old_status, v_old_plan
    ...
    update public.tenant_subscriptions
       set plan_id = coalesce(v_plan_id, plan_id),
           status = v_new_status,
           cancelled_at = case when v_action = 'CANCEL_SUBSCRIPTION' then now() when v_action = 'RESUME_SUBSCRIPTION' then null else cancelled_at end,
           expires_at = case when v_action = 'EXTEND_SUBSCRIPTION' then greatest(coalesce(expires_at, now()), now()) + make_interval(days => p_extension_days) when v_action = 'RESUME_SUBSCRIPTION' and (expires_at is null or expires_at < now()) then now() + interval '30 days' else expires_at end,
           updated_at = now()
    ...
    values (v_company_id, null, v_action, v_old_status, v_new_status, auth.uid(), 'platform_admin', jsonb_build_object('reason', p_reason, 'tenantSubscriptionId', p_target_id, 'previousPlan', v_old_plan, 'newPlan', v_new_plan, 'extensionDays', p_extension_days));
    ...
    values (auth.uid(), 'platform administrator', v_action, 'subscription', p_target_id::text, p_reason, 'CONFIRM:' || v_action || ':' || p_target_id::text, jsonb_build_object('plan', p_plan, 'subscriptionSource', v_source, 'companyId', v_company_id))
  ...
  select public.platform_admin_control_action(p_action, p_target_id, p_reason, p_company_id, p_plan, p_name, p_cycle, p_amount, p_status, p_extension_days)
    into v_result;
