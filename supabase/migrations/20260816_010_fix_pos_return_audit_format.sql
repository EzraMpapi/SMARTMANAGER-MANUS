...
BEGIN
  ...
  SELECT return_id INTO v_existing_return_id FROM public.pos_return_commits WHERE company_id = v_company_id AND idempotency_key = trim(p_idempotency_key) LIMIT 1;
  ...
  FOR v_item IN SELECT value FROM jsonb_array_elements(p_items) LOOP
    ...
    SELECT coalesce(sum((data ->> 'qty')::numeric), 0), max((data ->> 'price')::numeric)
     ...
     WHERE company_id = v_company_id AND data ->> 'transaction_id' = p_transaction_id::text AND data ->> 'item_sku' = v_sku;
    ...
    SELECT coalesce(sum((ri.data ->> 'qty')::numeric), 0)
     ...
     WHERE r.company_id = v_company_id AND r.data ->> 'transaction_id' = p_transaction_id::text AND ri.data ->> 'item_sku' = v_sku;
  ...
  VALUES (v_company_id, 'Return · ' || coalesce(v_doc_number, p_transaction_id::text), 'Completed', p_refund_total, p_reason, jsonb_build_object('transaction_id', p_transaction_id, 'reason', p_reason, 'refund_total', p_refund_total, 'cashier_id', v_user_id, 'cashier', v_cashier))
  ...
  FOR v_item IN SELECT value FROM jsonb_array_elements(p_items) LOOP
    ...
    SELECT id, coalesce(nullif(data ->> 'qty_on_hand', '')::numeric, nullif(data ->> 'quantity', '')::numeric, 0)
      INTO v_inventory_id, v_stock FROM public.inventory_items
     WHERE company_id = v_company_id AND data ->> 'sku' = v_sku FOR UPDATE;
  ...
  VALUES (v_company_id, 'POS return completed', 'Point of Sale', v_cashier, 'Return processed for receipt ' || coalesce(v_doc_number, p_transaction_id::text) || ': ' || p_refund_total::text || '.', coalesce(v_doc_number, p_transaction_id::text), jsonb_build_object('return_id', v_return_id, 'transaction_id', p_transaction_id, 'reason', p_reason, 'refund_total', p_refund_total));
