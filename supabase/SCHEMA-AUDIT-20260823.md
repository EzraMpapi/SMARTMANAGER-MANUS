...
| Evidence | Result |
|---|---:|
| Repository SQL migration files | 53 |
| Repository-declared tables parsed from migrations | 241 |
| Live Supabase migrations after reconciliation | 115 |
...
The Supabase security advisor still reports pre-existing hardening notices, including mutable function search paths, security-definer functions executable by `anon`, and an RLS-enabled Money Agent PIN table without policies. These are separate from missing-table reconciliation and require a dedicated privilege/RLS review before changing production behavior. They are intentionally not hidden by this schema audit.

## Verification sources

The audit was grounded in the Supabase connector results for `list_migrations`, `list_tables` with verbose metadata, the production `pg_proc` definition for `community_groups_assert_relationships()`, and the repository migration files under `supabase/migrations/`.
