-- FK advisor remediation wave 005 review/execution script.
-- Review-only operational artifact; CREATE INDEX CONCURRENTLY must run one statement at a time outside a transaction.
-- Selected from a fresh exact pg_catalog coverage query; recheck coverage and lock/storage budget immediately before execution.
-- Safety: no DROP INDEX, table data, foreign keys, RLS, grants, or policies.
