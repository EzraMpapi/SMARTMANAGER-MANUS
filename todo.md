...
- [x] Commit and push the audited authenticated SELECT and INSERT privilege grant migration and checklist update to GitHub (`71d60f9`).
...
## Build, Coverage, and Detailed RLS Audit
- [ ] Run a manual memory-capped Vite build with increased Node.js heap allocation and inspect complete artifacts.
- [x] Review regression coverage for mobile responsive views, including the 969 passed non-credential tests and focused responsive contracts.
- [x] Perform a bounded read-only audit of Supabase RLS, tenant predicates, policy roles, and helper-function posture.
- [x] Record the verification results and remaining blockers in the repository audit record.

## Policy-less RLS and Alternative Build Review
- [x] Verify ownership, grants, exposure paths, and backend-only usage for the six RLS-enabled tables with no policies.
- [x] Review the attached audit markdown and exact responsive source diff for correctness and unintended changes.
- [x] Attempt an alternative Vite chunking strategy and inspect whether complete frontend artifacts are generated; the sandbox terminated during chunk rendering before complete artifacts were emitted.
- [x] Record the findings and any unresolved security or resource limitations.

## GitHub Push and Supabase Schema Synchronization
- [ ] Review all intentional unpushed repository changes and exclude secrets, generated artifacts, and temporary failing probes.
- [ ] Compare every verified repository schema declaration and migration with the live authorized Supabase catalog.
...
## Deployment, Live Integration, and Rendering Verification
- [ ] Verify current hosting deployment status and probe safe live endpoints without submitting mutations.
- [ ] Run read-only end-to-end integration checks against the live Supabase instance and classify auth or credential limitations.
- [ ] Review the latest commit diff and capture representative responsive UI rendering evidence at desktop and phone viewports.
- [ ] Record deployment, integration, diff, and rendering results with any remaining blockers.


## Attachment 5 Forensic Audit and Safe Repair
- [x] Audit pasted_content_5 requirements against repository architecture, routes, modules, integrations, and Git state
- [x] Build frontend-to-Supabase feature, table, relationship, permission, and RLS coverage map
- [x] Audit live Supabase schema objects, RLS, tenant isolation, storage, functions, triggers, and indexes without destructive changes
- [x] Repair only verified code, validation, error handling, persistence, and workflow gaps
- [x] Apply only genuinely missing idempotent schema objects with tenant isolation and RLS safeguards (no missing object was proven; no DDL applied)
- [x] Run module, CRUD-contract, responsive, authentication, subscription, integration, and security validation (available automated coverage passed; live tenant CRUD remains gated)
