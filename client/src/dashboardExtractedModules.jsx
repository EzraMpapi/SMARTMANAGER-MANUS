  ...
  return (
    <div className="space-y-4">
      ...
      {tab==="deposits" && (
        <div className="space-y-3">
          <div className="grid grid-cols-3 gap-3">
            {[["Total FD Value","TZS "+money(fdTotal)+"k",BANK_GOLD],["Active FDs",rowsOf(fds).filter(f=>f.status==="Active").length,"#2563EB"],["Total Interest Earned","TZS "+money(fds.rows.reduce((s,f)=>s+f.interestEarned,0))+"k","#16A34A"]].map(([l,v,col])=>(
              <div key={l} className="bg-white rounded-xl border border-slate-200/80 p-4 text-center"><p className="text-[11px] text-slate-400 uppercase tracking-wide mb-1">{l}</p><p className="text-[20px] font-bold" style={{color:col}}>{v}</p></div>
            ))}
          </div>
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
  ...
  return (
    <div className="space-y-4">
      ...
      {tab==="menu" && (
        <div className="space-y-3">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[["Menu Items",menuItems.rows.length,"#B91C1C"],["Available",rowsOf(menuItems).filter(m=>m.available).length,"#16A34A"],["Popular",rowsOf(menuItems).filter(m=>m.popular).length,"#F59E0B"],["Categories",MENU_CATEGORIES.length,"#7C3AED"]].map(([l,v,col])=>(
