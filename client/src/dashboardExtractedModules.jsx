...
function CommunityGroupsModule({ currentUser, canManage = false, onOpenMemberInvitation }) {
  ...
  const activeGroup = rowsOf(groups).find((row) => String(row.id) === String(activeGroupId) || String(row.dbId) === String(activeGroupId));
  ...
  const totalFund = paidContributions + savingsDeposits + activeIncome.reduce((sum, row) => sum + row.amount, 0) + rowsOf(fundraising).filter((row) => String(row.groupId) === String(activeGroupId)).reduce((sum, row) => sum + row.amount, 0) - savingsWithdrawals - welfarePaid - expenseTotal - loanPortfolio;
  const fmtMember = (memberId) => rowsOf(members).find((row) => String(row.id) === String(memberId) || String(row.dbId) === String(memberId))?.fullName || "Unassigned member";
  const fmtGroup = (groupId) => rowsOf(groups).find((row) => String(row.id) === String(groupId) || String(row.dbId) === String(groupId))?.name || "Unassigned group";
  ...
  const GroupSelect = ({ value, onChange }) => <select className={inputClass} value={value} onChange={(event) => onChange(event.target.value)}><option value="">Select group…</option>{rowsOf(groups).map((row) => <option key={row.id || row.dbId} value={row.id || row.dbId}>{row.name}</option>)}</select>;
  const MemberSelect = ({ value, onChange, groupId = activeGroupId }) => <select className={inputClass} value={value} onChange={(event) => onChange(event.target.value)}><option value="">Select member…</option>{rowsOf(members).filter((row) => !groupId || String(row.groupId) === String(groupId)).map((row) => <option key={row.id || row.dbId} value={row.id || row.dbId}>{row.fullName} · {row.memberNumber}</option>)}</select>;
...
function LegacyHealthcareClinicModule({ currentUser, company }) {
  ...
  const nextMrn = () => "MRN-001-" + String(rowsOf(patients).length + 1).padStart(6, "0");
  ...
  async function saveAppointment() {
    ...
    const pat = rowsOf(patients).find((p) => p.id === aForm.patientId);
    const doc = rowsOf(doctors).find((d) => d.id === aForm.doctorId);
  ...
  async function openVisit(appt) {
    const existing = rowsOf(visits).find((v) => v.patientId === appt.patientId && v.status === "Open");
  ...
  async function issuePrescription() {
    ...
    const pat = rowsOf(patients).find((p) => p.id === rxForm.patientId);
    const doc = rowsOf(doctors).find((d) => d.id === rxForm.doctorId);
  ...
  async function createReport() {
    ...
    const pat = rowsOf(patients).find((p) => p.id === repForm.patientId);
    const doc = rowsOf(doctors).find((d) => d.id === repForm.doctorId);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-2xl overflow-hidden px-5 py-5" style={{ background: "linear-gradient(135deg,#1B4DE4 0%,#2D6BE4 50%,#0F9D8E 100%)" }}>
        <div className="flex items-center justify-between gap-3 flex-wrap">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            <button onClick={()=>{const co=window.__smartManagerCompany||{};printReport("Patient Registry",`<div class="kpi-grid"><div class="kpi"><div class="kpi-label">Patients</div><div class="kpi-value" style="color:#0891B2">${rowsOf(patients).length}</div></div></div><table><thead><tr><th>MRN</th><th>Name</th><th>Gender</th><th>Blood Type</th><th>Phone</th><th>Insurance</th></tr></thead><tbody>${rowsOf(patients).map((p,i)=>`<tr style="background:${i%2===0?"white":"#F8FAFB"}"><td class="bold">${p.mrn||p.id}</td><td>${(p.firstName||"")} ${(p.lastName||"")}</td><td>${p.gender||"—"}</td><td>${p.bloodType||"—"}</td><td>${p.phone||"—"}</td><td>${p.insuranceProvider||"—"}</td></tr>`).join("")}</tbody></table>`,co);}}
              className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-[12px] font-semibold text-white" style={{background:"rgba(255,255,255,.15)",border:"1px solid rgba(255,255,255,.3)"}}>
              <Printer size={13}/> PDF
            </button>
      ...
      {tab === "overview" && (
        <div className="space-y-4">
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
            {[
              { label:"Patients",         value: rowsOf(patients).length,                           sub:"Total registered",         color:"#1B4DE4", icon:Users,          bg:"linear-gradient(135deg,#1B4DE4,#4B79F5)" },
              { label:"Doctors",          value: rowsOf(doctors).filter(d=>d.status==="Active").length, sub:"Active practitioners",  color:"#0F9D8E", icon:Stethoscope,    bg:"linear-gradient(135deg,#0F9D8E,#26BFB1)" },
              { label:"Appointments",     value: rowsOf(appointments).length,                       sub:"Total booked",             color:"#7C3AED", icon:CalendarCheck,  bg:"linear-gradient(135deg,#7C3AED,#9F6FF0)" },
              { label:"Open Visits",      value: rowsOf(visits).filter(v=>v.status==="Open").length, sub:"Currently active",        color:"#059669", icon:Activity,       bg:"linear-gradient(135deg,#059669,#10B981)" },
              { label:"Prescriptions",    value: rowsOf(prescriptions).filter(p=>p.status==="Active").length, sub:"Active today",  color:"#D97706", icon:Pill,           bg:"linear-gradient(135deg,#D97706,#F59E0B)" },
              { label:"Lab Orders",       value: rowsOf(labOrders).length,                          sub:"Pending results",          color:"#DC2626", icon:FlaskConical,   bg:"linear-gradient(135deg,#DC2626,#EF4444)" },
            ].map((k) => (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              ...
              {["Scheduled","Confirmed","In Progress","Completed","Cancelled"].map((s) => {
                const n = rowsOf(appointments).filter(a=>a.status===s).length;
                return n > 0 ? (
            ...
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              ...
              <div className="space-y-2">
                {rowsOf(appointments).filter(a=>a.status!=="Cancelled"&&a.status!=="Completed").map((a) => (
                ...
                {rowsOf(appointments).filter(a=>a.status!=="Cancelled"&&a.status!=="Completed").length === 0 && <p className="text-[12px] text-slate-400 py-4 text-center">No upcoming appointments</p>}
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              ...
              <div className="space-y-2">
                ...
                {rowsOf(visits).length === 0 && <p className="text-[12px] text-slate-400 py-4 text-center">No visits yet</p>}
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              ...
              {(() => {
                const apptData = ["Scheduled","Confirmed","In Progress","Completed","Cancelled"].map((s,i)=>({
                  name:s, value:rowsOf(appointments).filter(a=>a.status===s).length,
            ...
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              ...
              {(() => {
                const doctorLoad = rowsOf(doctors).map((d,i)=>({
                  name: d.fullName?.split(" ").pop()||d.lastName||"Dr.",
                  value: rowsOf(appointments).filter(a=>a.doctorId===d.id).length,
      ...
      {tab === "patients" && (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <p className="text-[13px] text-slate-500">{rowsOf(patients).length} registered patients</p>
      ...
      {tab === "doctors" && (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <p className="text-[13px] text-slate-500">{rowsOf(doctors).length} practitioners</p>
      ...
      {tab === "appointments" && (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <p className="text-[13px] text-slate-500">{rowsOf(appointments).length} appointments</p>
      ...
      {tab === "visits" && (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <p className="text-[13px] text-slate-500">{rowsOf(visits).length} visits total</p>
                ...
                {rowsOf(visits).map((v) => (
                  <tr key={v.id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/50">
                    ...
                    <td className="px-4 py-3">
                      ...
                      <button onClick={() => { const pat=rowsOf(patients).find(p=>p.id===v.patientId); setReviewPatient(pat); setTab("doctorreview"); }} className="ml-1 text-[11px] font-medium text-[#1B4DE4] hover:underline">Review</button>
                    </td>
                  </tr>
                ))}
                {rowsOf(visits).length===0 && <tr><td colSpan={7} className="px-4 py-10 text-center text-slate-400">No visits yet. Open a visit from the Appointments tab.</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <select className={inputClass + " text-[12.5px]"} value={reviewDoc?.id||""} onChange={e=>setReviewDoc(rowsOf(doctors).find(d=>d.id===e.target.value)||null)}>
                {rowsOf(doctors).map(d=><option key={d.id} value={d.id}>{d.fullName}</option>)}
              </select>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5">
            <div className="flex items-start justify-between mb-4">
              ...
              <select className={inputClass + " text-[12.5px]"} value={reviewPatient?.id||""} onChange={e=>setReviewPatient(rowsOf(patients).find(p=>p.id===e.target.value)||null)}>
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
            ...
            {rowsOf(labOrders).filter(l=>l.patientId===reviewPatient?.id).length === 0
              ? <p className="text-[12.5px] text-slate-400">No analyses for this patient yet.</p>
              : rowsOf(labOrders).filter(l=>l.patientId===reviewPatient?.id).map((lo) => (
            ...
            {reviewTab === "prescribe" && (
              <div className="space-y-3">
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  <FormField label="Doctor"><select className={inputClass} value={reviewDoc?.id||""} onChange={e=>setReviewDoc(rowsOf(doctors).find(d=>d.id===e.target.value)||null)}>{rowsOf(doctors).map(d=><option key={d.id} value={d.id}>{d.fullName}</option>)}</select></FormField>
      ...
      {tab === "reports" && (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <p className="text-[13px] text-slate-500">{rowsOf(reports).length} medical reports</p>
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
            <table className="w-full text-[12.5px]"><thead><tr className="border-b border-slate-100 bg-slate-50">{["Report ID","Patient","Doctor","Title","Date","Status",""].map(h=><th key={h} className="px-4 py-3 text-left text-[10.5px] font-medium uppercase tracking-wide text-slate-400">{h}</th>)}</tr></thead>
              <tbody>{rowsOf(reports).length===0?<tr><td colSpan={7} className="px-4 py-10 text-center text-slate-400">No reports yet.</td></tr>:rowsOf(reports).map((r)=>(
      ...
      {tab === "prescriptions" && (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <p className="text-[13px] text-slate-500">{rowsOf(prescriptions).length} prescriptions issued</p>
            <div className="flex gap-2">
              <button onClick={()=>downloadCSV("prescriptions",rowsOf(prescriptions).map(rx=>({ID:rx.id,Patient:rx.patient,Doctor:rx.doctor,Date:rx.date,Status:rx.status,Notes:rx.notes||""})),[{key:"ID",label:"ID"},{key:"Patient",label:"Patient"},{key:"Doctor",label:"Doctor"},{key:"Date",label:"Date"},{key:"Status",label:"Status"},{key:"Notes",label:"Notes"}])}
          ...
          <div className="space-y-3">
            ...
            {rowsOf(prescriptions).length===0&&<div className="bg-white rounded-xl border border-slate-200/80 p-10 text-center"><Pill size={28} className="text-slate-200 mx-auto mb-2"/><p className="text-slate-400">No prescriptions yet.</p></div>}
          </div>
        </div>
      )}
          ...
          {subTab==="orders" && (
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
              ...
              <table className="w-full text-[12.5px]"><thead><tr className="border-b border-slate-100 bg-slate-50">{["Order #","Patient","Tests","Date","Status"].map(h=><th key={h} className="px-4 py-3 text-left text-[10.5px] font-medium uppercase tracking-wide text-slate-400">{h}</th>)}</tr></thead>
                <tbody>{rowsOf(labOrders).length===0?<tr><td colSpan={5} className="px-4 py-10 text-center text-slate-400">No lab orders. Order from Doctor Review tab.</td></tr>:rowsOf(labOrders).map((lo)=>(
      ...
      {tab === "pharmacy" && (
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-white rounded-xl border border-slate-200/80 p-4"><p className="text-[11px] text-slate-400 uppercase tracking-wide mb-1">Active Prescriptions</p><p className="text-[22px] font-bold" style={{color:HC_BLUE}}>{rowsOf(prescriptions).filter(p=>p.status==="Active").length}</p></div>
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
            ...
            <div className="space-y-2">
              {rowsOf(prescriptions).filter(p=>p.status==="Active").map((rx) => (
              ...
              {rowsOf(prescriptions).filter(p=>p.status==="Active").length===0 && <p className="text-[12.5px] text-slate-400 py-6 text-center">No prescriptions pending dispensing.</p>}
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{background:"rgba(0,0,0,0.45)"}} onClick={()=>setModalType(null)}>
          <div className="bg-white rounded-2xl p-6 w-full max-w-2xl mx-4 shadow-2xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
            ...
            {modalType === "appointment" && (<>
              ...
              <div className="grid grid-cols-2 gap-3">
                <FormField label="Patient *"><select className={inputClass} value={aForm.patientId} onChange={e=>setAForm({...aForm,patientId:e.target.value})}><option value="">Select a patient...</option>{rowsOf(patients).map(p=><option key={p.id} value={p.id}>{p.fullName} ({p.mrn})</option>)}</select></FormField>
                <FormField label="Doctor *"><select className={inputClass} value={aForm.doctorId} onChange={e=>{const d=rowsOf(doctors).find(d=>d.id===e.target.value);setAForm({...aForm,doctorId:e.target.value,fee:d?.fee||0});}}><option value="">Select a doctor...</option>{rowsOf(doctors).map(d=><option key={d.id} value={d.id}>{d.fullName}</option>)}</select></FormField>
            ...
            {modalType === "prescription" && (<>
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                <FormField label="Patient *"><select className={inputClass} value={rxForm.patientId} onChange={e=>setRxForm({...rxForm,patientId:e.target.value})}><option value="">Select patient...</option>{rowsOf(patients).map(p=><option key={p.id} value={p.id}>{p.fullName}</option>)}</select></FormField>
                <FormField label="Doctor *"><select className={inputClass} value={rxForm.doctorId} onChange={e=>setRxForm({...rxForm,doctorId:e.target.value})}><option value="">Select doctor...</option>{rowsOf(doctors).map(d=><option key={d.id} value={d.id}>{d.fullName}</option>)}</select></FormField>
              </div>
            ...
            {modalType === "report" && (<>
              ...
              <div className="space-y-3">
                <FormField label="From a finished visit">
                  <select className={inputClass} value={repForm.visitId} onChange={e=>{const v=rowsOf(visits).find(v=>v.id===e.target.value);setRepForm({...repForm,visitId:e.target.value,patientId:v?.patientId||"",doctorId:v?.doctorId||""});}}>
                    <option value="">Select visit...</option>{rowsOf(visits).filter(v=>v.status==="Closed").map(v=><option key={v.id} value={v.id}>{v.id} &middot; {v.patient} &middot; {v.date?.slice(0,10)}</option>)}
                  </select>
                </FormField>
                <div className="grid grid-cols-2 gap-3">
                  <FormField label="Patient *"><select className={inputClass} value={repForm.patientId} onChange={e=>setRepForm({...repForm,patientId:e.target.value})}><option value="">Select...</option>{rowsOf(patients).map(p=><option key={p.id} value={p.id}>{p.fullName} ({p.mrn})</option>)}</select></FormField>
                  <FormField label="Doctor *"><select className={inputClass} value={repForm.doctorId} onChange={e=>setRepForm({...repForm,doctorId:e.target.value})}><option value="">Select...</option>{rowsOf(doctors).map(d=><option key={d.id} value={d.id}>{d.fullName}</option>)}</select></FormField>
                </div>
...
function LegacyHotelManagementModule({ currentUser, company }) {
  ...
  const occupied   = rowsOf(rooms).filter(r=>r.status==="Occupied").length;
  const available  = rowsOf(rooms).filter(r=>r.status==="Available").length;
  const cleaning   = rowsOf(rooms).filter(r=>r.status==="Cleaning").length;
  const occupancy  = rowsOf(rooms).length>0?(occupied/rowsOf(rooms).length*100).toFixed(0):0;
  const revenue    = rowsOf(bookings).filter(b=>b.status==="Checked Out"||b.status==="Active").reduce((s,b)=>s+b.paid,0);
  const adr        = occupied>0?(revenue/Math.max(occupied,1)).toFixed(0):0; // Average Daily Rate
  const revPAR     = rowsOf(rooms).length>0?(Number(adr)*occupied/rowsOf(rooms).length).toFixed(0):0; // Revenue Per Available Room
  ...
  async function doCheckIn() {
    ...
    const room   = rowsOf(rooms).find(r=>r.id===checkInForm.roomId);
  ...
  function doCheckOut(bookingId) {
    const bkg = rowsOf(bookings).find(b=>b.id===bookingId);
    if (!bkg) return;
  ...
  const houseKeepingTasks = rowsOf(rooms).filter(r=>r.status==="Cleaning"||r.status==="Occupied");
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl px-6 py-5 relative overflow-hidden" style={{background:`linear-gradient(135deg,#0F172A 0%,${HTL_BLUE} 45%,#1e40af 100%)`}}>
        ...
        <div className="relative flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex gap-2 flex-wrap">
            {[["Rooms",rowsOf(rooms).length,""],["Occupancy",occupancy+"%",""],["ADR","USD "+adr,""],["RevPAR","USD "+revPAR,""]].map(([l,v])=>(
      ...
      {tab==="overview" && (
        <div className="space-y-4">
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
            {[
              {l:"Total Rooms",  v:rowsOf(rooms).length,  c:HTL_BLUE,  I:Bed,          sub:"In property"},
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              ...
              {roomsByType.map(type=>{
                const typeRooms=rowsOf(rooms).filter(r=>r.type===type);
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
            ...
            {rowsOf(bookings).filter(b=>b.status==="Active").length===0
              ?<p className="text-slate-400 text-center py-4">No guests currently checked in</p>
              :<div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {rowsOf(bookings).filter(b=>b.status==="Active").map(b=>(
        ...
        <div className="space-y-4">
          <div className="flex gap-2 flex-wrap">
            {Object.entries(statusConfig).map(([s,cfg])=>{
              const n=rowsOf(rooms).filter(r=>r.status===s).length;
              return(
              ...
              <div key={type}>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
                  {typeRooms.map(r=>{
                    ...
                    const bkg=rowsOf(bookings).find(b=>b.room===r.number&&b.status==="Active");
                    return(
      ...
      {tab==="checkin" && (
        <div className="space-y-4">
          ...
          {showCheckIn&&(
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                ...
                <FormField label="Room *"><select className={inputClass} value={checkInForm.roomId} onChange={e=>setCheckInForm({...checkInForm,roomId:e.target.value})}><option value="">Select available room...</option>{rowsOf(rooms).filter(r=>r.status==="Available").map(r=><option key={r.id} value={r.id}>Room {r.number} — {r.type} (USD {r.price}/night)</option>)}</select></FormField>
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
            <div className="px-4 py-3 border-b border-slate-100"><p className="text-[13.5px] font-semibold text-[#111827]">Currently Checked-In ({rowsOf(bookings).filter(b=>b.status==="Active").length} guests)</p></div>
            <table className="w-full text-[12.5px]">
              ...
              <tbody>{rowsOf(bookings).filter(b=>b.status==="Active").map(b=>(
            ...
            {rowsOf(bookings).filter(b=>b.status==="Active").length===0&&<p className="text-center text-slate-400 py-8">No guests currently checked in</p>}
          </div>
        </div>
      )}
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
            ...
            <div className="divide-y divide-slate-50">
              {rowsOf(rooms).filter(r=>r.status==="Cleaning"||r.status==="Occupied").map(r=>{
                const sc=statusConfig[r.status]||statusConfig.Available;
                return(
              ...
              {rowsOf(rooms).filter(r=>r.status==="Cleaning"||r.status==="Occupied").length===0&&<p className="text-center text-slate-400 py-10">All rooms are clean and available 🌟</p>}
            </div>
          </div>
        </div>
      )}
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              ...
              {roomsByType.map(type=>{
                const typeRevenue=rowsOf(bookings).filter(b=>b.type===type).reduce((s,b)=>s+b.paid,0);
                const total=rowsOf(bookings).reduce((s,b)=>s+b.paid,0);
            ...
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              ...
              {["Direct","Booking.com","Expedia","Airbnb","Phone"].map(src=>{
                const n=rowsOf(bookings).filter(b=>b.source===src).length;
                const total=rowsOf(bookings).length;
                if(!n)return null;
                return(
      ...
      {tab === "reports" && (() => {
        const revByType = ["Single","Double","Suite","Deluxe"].map(type=>{
          const typeRevenue = rowsOf(bookings).filter(b=>b.type===type&&(b.status==="Checked Out"||b.status==="Active"))
        ...
        const sourceData = ["Direct","Booking.com","Expedia","Airbnb","Phone"].map((src,i)=>({
          name:src, value:rowsOf(bookings).filter(b=>b.source===src).length,
            ...
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-[12px]">
                  ...
                  <tbody>
                    {rowsOf(bookings).slice(0,8).map((b,i)=>(
              ...
              <button
                onClick={()=>{
                  ...
                  if (!win) return;
                  const rows=rowsOf(bookings).map((b,i)=>`<tr style="background:${i%2===0?"#fff":"#F8FAFB"}">
...
function LegacyBankingMFIModule({ currentUser, company, onLoansLoad = null, onOpenStandingOrderWorkflow = null }) {
  ...
  const loanPortfolio = rowsOf(loans).filter(l=>l.status==="Active"||l.status==="Overdue").reduce((s,l)=>s+l.balance,0);
  const npls          = rowsOf(loans).filter(l=>l.status==="Overdue"||l.status==="Defaulted");
  const nplAmount     = npls.reduce((s,l)=>s+l.balance,0);
  const nplRatio      = loanPortfolio>0 ? (nplAmount/loanPortfolio*100).toFixed(2) : 0;
  const interestIncome = rowsOf(loans).reduce((s,l)=>s+(l.principal*(l.rate/100)/12),0);
  const fdTotal       = rowsOf(fds).filter(f=>f.status==="Active").reduce((s,f)=>s+f.amount,0);
  const todayTxns     = rowsOf(txns).filter(t=>t.date?.startsWith(TODAY.toISOString().slice(0,10)));

  const nextAccNo = () => String(1000000000 + rowsOf(accounts).length + 1);
  ...
  async function doTellerOp(opType) {
    ...
    const acc = rowsOf(accounts).find(a => a.accountNo === accountNo);
    ...
    if (opType==="Transfer Out" && toAccountNo) {
      const toAcc = rowsOf(accounts).find(a => a.accountNo === toAccountNo);
      if (toAcc) {
  ...
  async function disburseLoan() {
    ...
    const acc = rowsOf(accounts).find(a=>a.accountNo===loanForm.clientAccountNo);
  ...
  const filteredAccounts = rowsOf(accounts).filter(a => !searchAcc || a.name.toLowerCase().includes(searchAcc.toLowerCase()) || a.accountNo.includes(searchAcc) || a.type.toLowerCase().includes(searchAcc.toLowerCase()));

  // Account mini-statement modal
  const AccStatement = ({acc}) => {
    const accTxns = rowsOf(txns).filter(t => t.accountNo === acc.accountNo);
    return (
    ...
    <div className="space-y-4">
        ...
        <div className="relative flex items-start justify-between flex-wrap gap-4">
          ...
          <div className="flex gap-2 flex-wrap">
            <div className="rounded-xl px-4 py-2.5 text-center" style={{background:"rgba(255,255,255,.1)"}}>
              <p className="text-[20px] font-black text-white">{rowsOf(accounts).filter(a=>a.status==="Active").length}</p>
      ...
      {tab==="dashboard" && (
        <div className="space-y-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              {l:"Total Deposits",   v:"TZS "+money(totalDeposits)+"k",   sub:rowsOf(accounts).filter(a=>a.status==="Active").length+" active accounts", c:BANK_BLUE,  I:PiggyBank },
              {l:"Loan Portfolio",   v:"TZS "+money(loanPortfolio)+"k",   sub:rowsOf(loans).filter(l=>l.status==="Active").length+" active loans",       c:"#059669",  I:Landmark },
              {l:"Fixed Deposits",   v:"TZS "+money(fdTotal)+"k",         sub:rowsOf(fds).filter(f=>f.status==="Active").length+" accounts",             c:BANK_GOLD,  I:Archive },
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              ...
              {ACCOUNT_TYPES.slice(0,5).map(type=>{
                const typeTotal=rowsOf(accounts).filter(a=>a.type===type).reduce((s,a)=>s+a.balance,0);
            ...
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              ...
              {[["Performing","Active","#16A34A"],["Overdue","Overdue","#F59E0B"],["Defaulted","Defaulted","#EF4444"],["Closed","Closed","#6B7280"]].map(([l,s,col])=>{
                const n=rowsOf(loans).filter(x=>x.status===s).length;
                const amt=rowsOf(loans).filter(x=>x.status===s).reduce((sum,x)=>sum+x.balance,0);
                return(
      ...
      {tab==="accounts" && (
        <div className="space-y-3">
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
            ...
            <div className="px-4 py-2.5 border-t border-slate-100 bg-slate-50 text-[11.5px] text-slate-400">{filteredAccounts.length} of {rowsOf(accounts).length} accounts · Total: TZS {money(totalDeposits)}k</div>
          </div>
        </div>
      )}
      ...
      {tab==="teller" && (
        <div className="space-y-4">
            ...
            <div className="p-6">
              <div className="max-w-lg mx-auto space-y-4">
                ...
                {tellerForm.accountNo && (() => {
                  const acc = rowsOf(accounts).find(a=>a.accountNo===tellerForm.accountNo);
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
            <div className="px-4 py-3 border-b border-slate-100"><p className="text-[13.5px] font-semibold text-[#111827]">Today Teller Journal ({rowsOf(txns).length} transactions)</p></div>
            <table className="w-full text-[12.5px]">
      ...
      {tab==="loans" && (
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[["Active Loans",rowsOf(loans).filter(l=>l.status==="Active").length,"#2563EB"],["Portfolio","TZS "+money(loanPortfolio)+"k","#059669"],["NPLs",npls.length,"#EF4444"],["Collected Est.","TZS "+money(rowsOf(loans).filter(l=>l.status==="Active").reduce((s,l)=>s+l.installment,0))+"k","#7C3AED"]].map(([l,v,col])=>(
              <div key={l} className="bg-white rounded-xl border border-slate-200/80 p-4 text-center"><p className="text-[11px] text-slate-400 uppercase tracking-wide mb-1">{l}</p><p className="text-[20px] font-bold" style={{color:col}}>{v}</p></div>
            ))}
          </div>
          ...
          {showLoanForm && (
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5 space-y-3">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                ...
                {loanForm.clientAccountNo && (() => { const a=rowsOf(accounts).find(x=>x.accountNo===loanForm.clientAccountNo); return a?<div className="col-span-2 py-2 px-3 rounded-lg bg-blue-50 text-[12.5px] font-semibold text-blue-800">{a.name} · {a.type}</div>:null; })()}
      ...
      {tab==="deposits" && (
        <div className="space-y-3">
          <div className="grid grid-cols-3 gap-3">
            {[["Total FD Value","TZS "+money(fdTotal)+"k",BANK_GOLD],["Active FDs",rowsOf(fds).filter(f=>f.status==="Active").length,"#2563EB"],["Total Interest Earned","TZS "+money(rowsOf(fds).reduce((s,f)=>s+f.interestEarned,0))+"k","#16A34A"]].map(([l,v,col])=>(
              <div key={l} className="bg-white rounded-xl border border-slate-200/80 p-4 text-center"><p className="text-[11px] text-slate-400 uppercase tracking-wide mb-1">{l}</p><p className="text-[20px] font-bold" style={{color:col}}>{v}</p></div>
            ))}
          </div>
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              ...
              ["Active Accounts",String(rowsOf(accounts).filter(a=>a.status==="Active").length),"#7C3AED"],
            ].map(([l,v,col])=>(
              <div key={l} className="bg-white rounded-xl border border-slate-200/80 p-4 text-center">
                <p className="text-[10.5px] text-slate-400 uppercase tracking-wide mb-1">{l}</p>
                <p className="text-[18px] font-bold" style={{color:col}}>{v}</p>
            ...
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              ...
              {(() => {
                const statusData = ["Active","Overdue","Closed","Written Off"].map((s,i)=>({
                  name:s, value:rowsOf(loans).filter(l=>l.status===s).length,
            ...
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
              ...
              {(() => {
                const typeData = ACCOUNT_TYPES.slice(0,6).map((type,i)=>({
                  name:type.replace(" ",""),
                  value:Math.round(rowsOf(accounts).filter(a=>a.type===type).reduce((s,a)=>s+(a.balance||0),0)/1000),
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5">
              ...
              <div className="space-y-2">
                {[["Interest on Loans (Monthly)",money(interestIncome)+"k","#16A34A"],["FD Interest Expense",money(rowsOf(fds).reduce((s,f)=>s+(f.amount*f.rate/100/12),0))+"k","#EF4444"],["Net Interest Margin",money(interestIncome-rowsOf(fds).reduce((s,f)=>s+(f.amount*f.rate/100/12),0))+"k","#2563EB"]].map(([l,v,col])=>(
...
function LegacyRestaurantModule({ currentUser, company }) {
  ...
  // Analytics
  const todayOrders  = rowsOf(orders).filter(o=>o.status!=="Cancelled");
  const todayRevenue = todayOrders.filter(o=>o.status==="Paid").reduce((s,o)=>s+o.total,0);
  const pendingOrders= rowsOf(orders).filter(o=>o.status==="Preparing"||o.status==="Ready");
  const occupiedTbls = rowsOf(tables).filter(t=>t.status==="Occupied").length;
  const occupancy    = rowsOf(tables).length>0?(occupiedTbls/rowsOf(tables).length*100).toFixed(0):0;
  ...
  async function placeOrder() {
    ...
    const tbl = rowsOf(tables).find(t=>t.id===activeTable);
    const row = {
  ...
  async function updateOrderStatus(orderId, status) {
    ...
    if (status==="Paid") {
      const ord = rowsOf(orders).find(o=>o.id===orderId);
      if (ord) {
  ...
  async function addReservation() {
    ...
    if (resvForm.table) {
      const tblId = rowsOf(tables).find(t=>t.number===resvForm.table)?.id;
  ...
  const filteredMenu = menuCat==="All" ? menuItems.rows : rowsOf(menuItems).filter(m=>m.category===menuCat);
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl px-6 py-5 relative overflow-hidden" style={{background:"linear-gradient(135deg,#7F1D1D 0%,#B91C1C 40%,#C2410C 100%)"}}>
        ...
        <div className="relative flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex gap-2 flex-wrap">
            {[["Tables",rowsOf(tables).length],["Occupied",occupiedTbls+" ("+occupancy+"%)"],["Active Orders",pendingOrders.length],["Revenue",TZS_FMT(todayRevenue)]].map(([l,v])=>(
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {Object.entries({Available:"#16A34A",Occupied:"#EF4444",Reserved:"#3B82F6",Cleaning:"#F59E0B"}).map(([s,col])=>{
              const n=rowsOf(tables).filter(t=>t.status===s).length;
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
            ...
            {TABLE_ZONES.map(zone=>{
              const zoneTables = rowsOf(tables).filter(t=>t.zone===zone);
                ...
                <div key={zone} className="mb-5 last:mb-0">
                  ...
                  <div className="flex flex-wrap gap-3">
                    {zoneTables.map(t=>{
                      ...
                      const ord = t.currentOrder ? rowsOf(orders).find(o=>o.id===t.currentOrder) : null;
                      return (
              ...
              <div className="p-4 border-b border-slate-100" style={{background:RST_RED}}>
                ...
                <div className="flex gap-2 mt-2">
                  <select className="flex-1 bg-white/20 text-white border border-white/30 rounded-lg px-2 py-1.5 text-[11.5px]" value={activeTable||""} onChange={e=>setActiveTable(e.target.value)}>
                    ...
                    {rowsOf(tables).filter(t=>t.status==="Available"||t.status===activeTable).map(t=><option key={t.id} value={t.id}>{t.number} — {t.seats} seats ({t.zone})</option>)}
                  </select>
      ...
      {tab==="kitchen" && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    ...
                    <div className="space-y-2.5 mb-4">
                      {o.items.map((item,i)=>(
                        <div key={i} className="flex items-center gap-3">
                          <span className="text-[22px]">{rowsOf(menuItems).find(m=>m.id===item.id)?.image||"🍽️"}</span>
            ...
            {rowsOf(orders).filter(o=>kitchenTab==="active"?o.status==="Preparing":kitchenTab==="ready"?o.status==="Ready":o.status!=="Cancelled").length===0&&(
      ...
      {tab==="menu" && (
        <div className="space-y-3">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[["Menu Items",rowsOf(menuItems).length,"#B91C1C"],["Available",rowsOf(menuItems).filter(m=>m.available).length,"#16A34A"],["Popular",rowsOf(menuItems).filter(m=>m.popular).length,"#F59E0B"],["Categories",MENU_CATEGORIES.length,"#7C3AED"]].map(([l,v,col])=>(
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
            ...
            <table className="w-full text-[12.5px]">
              ...
              <tbody>{rowsOf(menuItems).map(item=>{
          ...
          {showResvForm&&(
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5 space-y-3">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                ...
                <FormField label="Table"><select className={inputClass} value={resvForm.table} onChange={e=>setResvForm({...resvForm,table:e.target.value})}><option value="">Select table...</option>{rowsOf(tables).filter(t=>t.status==="Available").map(t=><option key={t.id} value={t.number}>{t.number} — {t.seats} seats ({t.zone})</option>)}</select></FormField>
      ...
      {tab==="reports" && (
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between flex-wrap gap-2">
            ...
            <div className="flex gap-2">
              <button onClick={()=>downloadCSV("restaurant-orders",rowsOf(orders).map(o=>({
              ...
              <button onClick={()=>{
                const co=window.__smartManagerCompany||{};
                const top=rowsOf(menuItems).map(mi=>{
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
            ...
            {rowsOf(menuItems).map(m=>{
              ...
              if(!sold)return null;
              const maxSold=Math.max(...rowsOf(menuItems).map(mi=>orders.rows.flatMap(o=>o.items).filter(i=>i.id===mi.id).reduce((s,i)=>s+i.qty,0)));
              return(
          ...
          {(() => {
            const catRev = rowsOf(menuItems).reduce((m,item)=>{
          ...
          {/* Print bill button */}
          {activeTable && (() => {
            const tblOrder = rowsOf(orders).find(o=>o.tableId===activeTable&&o.status!=="Paid"&&o.status!=="Cancelled");
            if (!tblOrder) return null;
            return (
              <div className="bg-[#FEF2F2] border border-[#FECACA] rounded-xl p-3.5 flex items-center justify-between">
                <div>
                  <p className="text-[13px] font-bold text-[#B91C1C]">Active order on Table {rowsOf(tables).find(t=>t.id===activeTable)?.number}</p>
...
function EmployeePortal({ currentUser, company, employees, leaveRequests, canManage, setInviteInput = () => {}, setInviteError = () => {} }) {
  ...
  const myAttendance = useMemo(()=>
    rowsOf(attendance).filter(a=>(self?.name||currentUser.name).toLowerCase().includes(a.employee?.toLowerCase()||"")||
