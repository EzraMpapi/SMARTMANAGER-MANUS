...
function Sparkline({ values, tone = "#089669" }) {
  ...
  if (valid.length < 2 || valid.every((value) => value === 0)) {
    return <span className="mt-2 block h-5 border-t border-dashed border-slate-200" aria-label="No comparable confirmed trend" />;
  }
  const min = Math.min(...valid);
  const max = Math.max(...valid);
  const span = max - min || 1;
  const points = valid.map((value, index) => `${(index / (valid.length - 1)) * 116 + 2},${27 - ((value - min) / span) * 20}`).join(" ");
  return <svg className="mt-2 h-5 w-full overflow-visible" viewBox="0 0 120 30" role="img" aria-label="Confirmed trend sparkline"><polyline fill="none" stroke={tone} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" points={points} /><circle cx="118" cy={27 - ((valid[valid.length - 1] - min) / span) * 20} r="2.1" fill={tone} /></svg>;
}
...
function MetricCard({ label, value, detail, icon: Icon, tone, series, onClick }) {
  ...
  return <button type="button" onClick={onClick} className="dashboard-reference-kpi group min-h-[132px] rounded-lg border border-slate-200/80 bg-white p-3 text-left shadow-[0_4px_12px_rgba(15,23,42,.035)] transition hover:-translate-y-0.5 hover:border-emerald-200 hover:shadow-[0_12px_24px_rgba(15,23,42,.08)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-600 focus-visible:ring-offset-2">
    <div className="flex items-start justify-between gap-1.5"><div className="min-w-0"><p className="whitespace-nowrap text-[8.5px] font-bold uppercase tracking-[.04em] text-slate-500">{label}</p><p className="mt-1 text-[15px] font-extrabold tracking-[-.035em] text-slate-950">{value}</p></div><span className="grid h-9 w-9 shrink-0 place-items-center rounded-full" style={{ color: tone, backgroundColor: `${tone}14` }}><Icon size={17} aria-hidden="true" /></span></div>
    <p className="mt-1.5 flex min-h-4 items-center gap-1 text-[9px] font-semibold"><span className={trendClass}>{trend.direction === "up" ? <ArrowUpRight size={11} /> : trend.direction === "down" ? <ArrowDownRight size={11} /> : null}</span><span className={trendClass}>{trend.label}</span></p>
    <p className="mt-0.5 truncate text-[9px] leading-4 text-slate-500" title={detail}>{detail}</p>
...
export function EnterpriseDashboardOverview({
  ...
  return <div className="enterprise-overview dashboard-reference-layout dashboard-reference-erp mx-auto w-full max-w-[1800px] space-y-3.5 pb-10">
    ...
    <section className="grid grid-cols-1 gap-2.5 min-[470px]:grid-cols-2 lg:grid-cols-3 xl:grid-cols-7" aria-label="Key performance indicators">
