...
function ProfileMenu({ currentUser, session, company, onSignOut, onNavigate, onOpenPasswordRecovery, roleChangeApprovalsQuery, onProfileUpdated, canManageBilling = false }) {
  ...
  const actions = [
    { label: "My profile", detail: "Identity and work settings", icon: UserRound, onClick: () => go("profile") },
  ...
  return (
    <div className="relative shrink-0">
      ...
      {open && (
        <section ref={menuRef} id="workspace-profile-menu" role="menu" aria-label="Account menu" className="absolute right-0 top-full z-50 mt-2 w-[min(92vw,320px)] overflow-hidden rounded-2xl border border-slate-200/90 bg-white p-1.5 shadow-[0_20px_55px_rgba(15,23,42,.18)]">
          ...
          <div className="mt-1 grid grid-cols-2 gap-1 border-t border-slate-100 p-1 pt-2"><button type="button" role="menuitem" onClick={() => { setOpen(false); onOpenPasswordRecovery?.(); }} disabled={!onOpenPasswordRecovery} className="inline-flex min-h-10 items-center justify-center gap-1.5 rounded-xl border border-slate-200 px-2 text-[10.5px] font-bold text-slate-600 transition hover:border-emerald-200 hover:bg-emerald-50 disabled:opacity-50"><KeyRound size={13} />Recovery</button><button type="button" role="menuitem" onClick={onSignOut} className="inline-flex min-h-10 items-center justify-center gap-1.5 rounded-xl border border-red-100 px-2 text-[10.5px] font-bold text-red-700 transition hover:bg-red-50"><ArrowRight size={13} />{session && !session.demo ? "Sign out" : "Exit demo"}</button></div>
        </section>
      )}
    </div>
  );
