...
function ProfileMenu({ currentUser, session, company, onSignOut, onNavigate, onOpenPasswordRecovery, roleChangeApprovalsQuery, onProfileUpdated, canManageBilling = false, topbar = false }) {
  ...
  return (
      ...
      <button ref={triggerRef} type="button" onClick={() => setOpen((value) => !value)} className={topbar ? "group grid h-10 w-10 place-items-center rounded-full border-0 bg-transparent p-0 text-left shadow-none transition hover:bg-emerald-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/40 sm:h-11 sm:w-11" : "group flex w-full min-h-11 items-center gap-2.5 rounded-xl border-0 bg-transparent px-1.5 py-1.5 text-left shadow-none transition hover:bg-white/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/50"} aria-expanded={open} aria-haspopup="menu" aria-controls="workspace-profile-menu" aria-label={`Open account identity center for ${displayName}`} title={displayName}>
        <Avatar profile={profile} name={displayName} size="md" />
      ...
      {open && (
        <section ref={menuRef} id="workspace-profile-menu" role="dialog" aria-label="Account identity center" className={`absolute ${topbar ? "right-0 top-full mt-2 sm:mt-3" : "bottom-full left-0 mb-2"} z-50 max-h-[calc(100dvh-5.5rem)] w-[min(92vw,360px)] overflow-hidden rounded-2xl border border-slate-200 bg-white p-1.5 shadow-[0_24px_60px_rgba(15,23,42,.18)]`}>
          ...
          <div className="mt-1 max-h-[min(52dvh,360px)] overscroll-contain overflow-y-auto p-1">
            {actions.map((action) => {
