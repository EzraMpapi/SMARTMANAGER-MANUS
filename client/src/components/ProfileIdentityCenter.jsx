...
function dateOfBirthInputValue(value) {
...
function profileFormFrom(profile) {
  return {
    ...
    dateOfBirth: dateOfBirthInputValue(profile?.dateOfBirth),
...
function LegacyProfileMenu({ currentUser, session, company, onSignOut, onNavigate, onOpenPasswordRecovery, roleChangeApprovalsQuery, onProfileUpdated, canManageBilling = false }) {
...
function ProfileMenu({ currentUser, session, company, onSignOut, onNavigate, onOpenPasswordRecovery, roleChangeApprovalsQuery, onProfileUpdated, canManageBilling = false, topbar = false }) {
  ...
  useEffect(() => {
    if (profileCenter.query.data) onProfileUpdatedRef.current?.(profileCenter.query.data);
  }, [profileCenter.query.data?.profile?.updatedAt]);
  ...
  const actions = [
    { label: "View My Profile", detail: "Identity and work settings", icon: UserRound, onClick: () => go("profile") },
    { label: "Security & access", detail: "Session and recovery controls", icon: ShieldCheck, onClick: () => go("profile", { profileTab: "security" }) },
    { label: "Preferences", detail: "Language and display settings", icon: Settings2, onClick: () => go("profile", { profileTab: "preferences" }) },
    { label: "Notifications", detail: "Workspace alerts", icon: Mail, onClick: () => go("notifications") },
    ...(canManageBilling ? [{ label: "Subscription & billing", detail: "Plan and payment controls", icon: CreditCard, onClick: () => go("billing") }] : []),
    { label: "Workspace settings", detail: pendingApprovals ? `${pendingApprovals} approval${pendingApprovals === 1 ? "" : "s"} pending` : "Permissions and workspace setup", icon: Building2, onClick: () => go("settings") },
  ];

  return (
      <div className={topbar ? "relative z-40 shrink-0" : "relative w-full"}>
      <button ref={triggerRef} type="button" onClick={() => setOpen((value) => !value)} className={topbar ? "group relative z-40 grid h-10 w-10 cursor-pointer place-items-center rounded-full border-0 bg-transparent p-0 text-left shadow-none transition hover:bg-emerald-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/40 sm:h-11 sm:w-11" : "group flex w-full min-h-11 items-center gap-2.5 rounded-xl border-0 bg-transparent px-1.5 py-1.5 text-left shadow-none transition hover:bg-white/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/50"} aria-expanded={open} aria-haspopup="menu" aria-controls="workspace-profile-menu" aria-label="Open account identity center" title={displayName}>
        <Avatar profile={profile} name={displayName} size="md" />
      ...
      {open && (
        <section ref={menuRef} id="workspace-profile-menu" role="dialog" aria-label="Account identity center" className={`absolute ${topbar ? "right-0 top-full mt-2 sm:mt-3" : "bottom-full left-0 mb-2"} z-50 max-h-[calc(100dvh-5.5rem)] w-[min(92vw,360px)] overflow-hidden rounded-2xl border border-slate-200 bg-white p-1.5 shadow-[0_24px_60px_rgba(15,23,42,.18)]`}>
          ...
          <div className="mt-1 max-h-[min(58vh,360px)] overscroll-contain overflow-y-auto p-1">
            {actions.map((action) => {
...
function PersonalTab({ profile, update, upload, remove, onSaved }) {
  ...
  async function saveProfile(event) {
    ...
    try { await update.mutateAsync({ ...form, dateOfBirth: form.dateOfBirth || null }); if (avatarDraft?.remove) await remove.mutateAsync(); else if (avatarDraft) await upload.mutateAsync({ mimeType: avatarDraft.mimeType, base64: avatarDraft.base64 }); setNotice({ type: "success", text: "Your profile changes were confirmed by the workspace." }); onSaved?.(); } catch (error) { setNotice({ type: "error", text: error?.message || "Your profile could not be saved." }); }
  }
