...
function ProfileMenu({ currentUser, session, company, onSignOut, onNavigate, onOpenPasswordRecovery, roleChangeApprovalsQuery, onProfileUpdated, canManageBilling = false, topbar = false }) {
  ...
  return (
      ...
      {open && (
        <section ref={menuRef} id="workspace-profile-menu" role="dialog" aria-label="Account identity center" className={`dashboard-profile-menu ${topbar ? "dashboard-topbar-profile-menu absolute right-0 top-full mt-2 sm:mt-3" : "bottom-full left-0 mb-2"} z-50 max-h-[calc(100dvh-5.5rem)] w-[min(92vw,360px)] overflow-hidden rounded-2xl border border-slate-200 bg-white p-1.5 shadow-[0_24px_60px_rgba(15,23,42,.18)]`}>
...
function PersonalTab({ profile, update, upload, remove, onSaved }) {
  ...
  async function saveProfile(event) {
    ...
    try { await update.mutateAsync({ ...form, dateOfBirth: dateOfBirthInputValue(form.dateOfBirth) || null }); if (avatarDraft?.remove) await remove.mutateAsync(); else if (avatarDraft) await upload.mutateAsync({ mimeType: avatarDraft.mimeType, base64: avatarDraft.base64 }); setNotice({ type: "success", text: "Your profile changes were confirmed by the workspace." }); onSaved?.(); } catch (error) { setNotice({ type: "error", text: error?.message || "Your profile could not be saved." }); }
  }
