...
function ProfileMenu({ currentUser, session, company, onSignOut, onNavigate, onOpenPasswordRecovery, roleChangeApprovalsQuery, onProfileUpdated, canManageBilling = false, topbar = false }) {
  ...
  return (
      <div className={topbar ? "dashboard-topbar-profile relative shrink-0" : "relative w-full"}>
      <button ref={triggerRef} type="button" onClick={() => setOpen((value) => !value)} className={topbar ? "group grid h-10 w-10 place-items-center rounded-full border-0 bg-transparent p-0 text-left shadow-none transition hover:bg-emerald-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/40 sm:h-11 sm:w-11" : "group flex w-full min-h-11 items-center gap-2.5 rounded-xl border-0 bg-transparent px-1.5 py-1.5 text-left shadow-none transition hover:bg-white/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/50"} aria-expanded={open} aria-haspopup="menu" aria-controls="workspace-profile-menu" aria-label="Open account identity center" title={displayName}>
        <Avatar profile={profile} name={displayName} size="md" />
